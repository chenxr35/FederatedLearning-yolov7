...
from utils.Update import fit_multiple_epoch, YoloDatasetSplit, save_model, eval_model
from utils.Fed import FedAvg, FedAvgM
from utils.sampling import voc_iid, voc_noniid
from utils.utils_FL import flatten_weights
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------#
    save_dir = f'logs/{ALGO}_{NUM_USERS}_iid{IID}'
    # ------------------------------------------------------------------#
    ...
    eval_flag = True
    eval_period = 10
    map_out_path = f".temp_map_out/{ALGO}_{NUM_USERS}_iid{IID}"
    # ------------------------------------------------------------------#
    ...
    parser.add_argument('--num_users', type=bool, default=NUM_USERS, help="number of users: K")
    parser.add_argument('--frac', type=float, default=1, help="the fraction of clients: C")
    parser.add_argument('--local_ep', type=int, default=5, help="the number of local epochs: E")
    parser.add_argument('--algo', type=str, default=ALGO, help="name of federated learning algorithm")
    parser.add_argument('--optim_type', type=str, default=optimizer_type, help="optimizer type")
    parser.add_argument('--cuda', type=bool, default=Cuda, help="whether use GPU or not")
    parser.add_argument('--fp16', type=bool, default=fp16, help="whether mixed-precision training or not")
    parser.add_argument('--local_rank', type=int, default=local_rank, help="GPU ID, -1 for CPU")
    ...
    parser.add_argument('--mu', type=float, default=0.001, help="mu parameter for FedProx algorithm")
    ...
    parser.add_argument('--lr', type=float, default=Init_lr, help="learning rate")

    args = parser.parse_args()
    args.global_ep = UnFreeze_Epoch // args.local_ep
    args.save_period = save_period // args.local_ep
    args.eval_period = eval_period // args.local_ep
    ...
    #   保存权值
    # ----------------------#
    ckpt_dir = os.path.join(save_dir, "checkpoints_" + str(time_str))
    ...
    if True:
        ...
        fl_gens = []
        if IID:
            dict_users = voc_iid(train_dataset, args.num_users)
        else:
            dict_users = voc_noniid(train_dataset, args.num_users)
        for idx in range(args.num_users):
            fl_train_dataset = YoloDatasetSplit(train_dataset, dict_users[idx])
            fl_gen = DataLoader(fl_train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                             pin_memory=True,
                             drop_last=True, collate_fn=yolo_dataset_collate, sampler=train_sampler)
            fl_gens.append(fl_gen)
        ...
        if local_rank == 0:
            eval_callback = EvalCallback(model, input_shape, anchors, anchors_mask, class_names, num_classes, val_lines,
                                         log_dir, Cuda, map_out_path=map_out_path, eval_flag=eval_flag, period=args.eval_period)
        else:
            eval_callback = None
        ...
        # ----------------------#
        if ALGO == 'scaffold':
            control_global = copy.deepcopy(model)
            control_global.cuda()
            ...
            delta_c = copy.deepcopy(model.state_dict())
            # sum of delta_y / sample size
            delta_x = copy.deepcopy(model.state_dict())
        ...
        #   联邦学习
        # ---------------------------------------#
        for epoch in range(Init_Epoch, args.global_ep):
            ...
            set_optimizer_lr(optimizer, lr_scheduler_func, epoch * args.local_ep)

            for idx in idxs_users:
                local_model = copy.deepcopy(model)

                if ALGO == 'scaffold':
                    w, loss, local_delta_c, local_delta, control_local_w = fit_multiple_epoch(args, local_model,yolo_loss, optimizer.state_dict(), fl_gens[idx], lr_scheduler_func, scaler, epoch, idx, local_controls[idx], control_global)
                    ...
                    w, loss, local_delta = fit_multiple_epoch(args, local_model, yolo_loss, optimizer.state_dict(), fl_gens[idx], lr_scheduler_func, scaler, epoch, idx)

                w_locals.append(copy.deepcopy(w))
                updates.append(copy.deepcopy(local_delta))
                num_examples.append(len(dict_users[idx]))
                loss_locals.append(copy.deepcopy(loss))

            # update global weights
            if ALGO == 'fedavg' or ALGO == 'fedprox' or ALGO == 'fedntd':
                w_glob = FedAvg(w_locals, num_examples)

            elif ALGO == 'fedavgm':
                # w_glob, m_flat = FedAvgM(w_locals, w_glob, num_examples, m_flat, args.avgm_beta)
                # Update server model
                update_avg = FedAvg(updates, num_examples)
                if not v:
                    v = copy.deepcopy(update_avg)
                else:
                    for key in v.keys():
                        v[key] = torch.add(update_avg[key], v[key], alpha=args.avgm_beta)
                for key in w_glob:
                    w_glob[key] = torch.sub(w_glob[key], v[key])
            ...
            if ema:
                # ema.updates = (epoch+1) * args.local_ep * epoch_step - 1
                ema.update(model)
            w_glob = model.state_dict()
            ...
            val_loss = eval_model(args, model, ema, gen_val, optimizer, yolo_loss, eval_callback, epoch)

            # #############
            # Save progress
            # #############

            save_model(args, model, ema, loss_history, loss_avg, val_loss, epoch, ckpt_dir)
